...
- Español
- Responde directo
- Devuelve SIEMPRE JSON con clave "text"
"""
...
async def consultar(data: ConsultaChat, db: Session = Depends(get_db)):
    ...
    if data.contexto_gastos:
        resumen_gastos = "\n".join(
            [f"- {g.get('item', 'Item')}: ${g.get('valor', 0)}" for g in data.contexto_gastos]
        )
    ...
    # --- modo ---
    instrucciones_tool = {
        "bolsa": "MODO BOLSA: analiza acciones y empresas.",
        "pensar": "MODO PENSAR: análisis profundo financiero.",
...
"""
        ...
        # elimina bloque markdown si existe
        if "```" in texto_limpio:
    ...
    return [
        {
            ...
            "ai_response": r.ai_response["text"]
